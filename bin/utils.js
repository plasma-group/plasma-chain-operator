...
async function getAccount() {
  ...
  if (!accounts.length) {
    console.log(
      'ERROR:'.red,
      'No account found! Create a new account with `plasma-chain account new`'
    )
    return
  }
  // Check if the account is plaintext
  let account = JSON.parse(
    fs.readFileSync(path.join(keystoreDirectory, accounts[0]))
  )
...
async function _unlockAccount(encryptedAccount) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    try {
      account = web3.eth.accounts.wallet.decrypt(
        [encryptedAccount],
        response.password
      )['0']
    } catch (err) {
